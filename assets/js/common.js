...
window.bookly.money = (amount, currency = 'USD') => {
  try { return new Intl.NumberFormat('en-US', { style: 'currency', currency: currency || 'USD' }).format(Number(amount || 0)); }
  catch { return `$${Number(amount || 0).toFixed(2)}`; }
};
window.bookly.requireUser = async (role = null) => {
  const { data: { user }, error } = await window.bookly.db.auth.getUser();
  if (error || !user) {
...
window.bookly.getProfile = async userId => {
  const { data, error } = await window.bookly.db.from('profiles').select('*').eq('id', userId).maybeSingle();
